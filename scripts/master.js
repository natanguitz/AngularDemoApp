...
app.controller("masterController", function($scope, $http , getAllMovies){
  ...
  $scope.allMovies = [];
  getAllMovies.loadMovies().then(response => $scope.allMovies = response.data);
});

app.controller("singleMovie", function($http, $scope, $routeParams,getAllMovies ){
  $scope.id = $routeParams.id;
  getAllMovies.LoadSingleMovie($scope.id).then(response => $scope.single = response.data );
});
...
app.config(function($routeProvider){
  ...
  $routeProvider
  .when("/allmovies",{templateUrl : "allmovies.html"})
  .when("/film/:id", {templateUrl: "film.html", controller: "singleMovie"})
